...
document.addEventListener('DOMContentLoaded', function () {
  const uploadButton = document.querySelector('#uploadButton'); // Target the upload button
  
  if (uploadButton) {
    uploadButton.addEventListener('click', async function (event) {
      event.preventDefault(); // Prevent default form submission behavior

      // Find the file input field (adjust the selector if necessary)
      const fileInput = document.querySelector('#fileInput'); // Add the ID to your file input field
      const csrfToken = document.querySelector('[name=csrfmiddlewaretoken]').value; // Get CSRF token from the page

      // Create FormData to send the file and CSRF token
      const formData = new FormData();
      formData.append('file', fileInput.files[0]); // Append the file
      formData.append('csrfmiddlewaretoken', csrfToken); // Append the CSRF token

      // Send the request to the Django endpoint
      try {
