...
async def sign_manifest(builder, signer, read_path, save_path):
    ...
    try:
        ...
        logger.error(f"FileNotFoundError: {fnf_error.filename} - {fnf_error.strerror}")
    except Exception as e:
...
async def process_file(file, **kwargs):
    ...
    try:
        ...
        # Save the file
        file.save(save_path)
        logger.info(f"The file is saved to: {save_path}")
    ...
    # create signer, with prod and dev paths
        ...
        if os.path.exists(save_path):
            os.remove(save_path)
        if os.path.exists(f"processed_files/{save_path}"):
            os.remove(f"processed_files/{save_path}")
